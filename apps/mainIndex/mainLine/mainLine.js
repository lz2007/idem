...
let mainIndex = avalon.define({
    ...
    within24: language_txt.index.within24,
    without24: language_txt.index.without24,
    ...
    topThree: [], //前三得分
    afterThree: ['', '', ''], //后三名得分
    dates: [language_txt.index.yesterdayRange, language_txt.index.weekRange, language_txt.index.monthRange],
    dateClass: 'active',
    weekClass: '',
    monthClass: '',
    ...
    updateRanking(postObj, id) { //更新排名情况
        ...
        ajax({
            ...
            for (var i = 0; i < 10; i++) {
                ...
                if (i < 3) {
                    mainIndex.topThree[i] = data[i].duration;
                } else {
                    mainIndex.afterThree[i] = data[i].duration;
                }
            }
...
function initPage() {
    //首页情况
    let objYes = getUpdatedPostObjAndID(yesterdayRange),
        objWeek = getUpdatedPostObjAndID(weekRange),
        objMon = getUpdatedPostObjAndID(monthRange);
...
function yesFormatter(a) {
    return (a['seriesName'] + '</br>' + a['name'] + ':00  :' + a['value'] + ' ' + language_txt.index.hours);
}
