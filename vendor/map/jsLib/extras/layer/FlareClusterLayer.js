...
dojo.provide("extras.layer.FlareClusterLayer");
...
dojo.declare("extras.layer.FlareClusterLayer",[esri.layers.GraphicsLayer],{
    constructor: function (options) {
        ...
        */
        ...
        options = options || {};
        this.spatialRef = options.spatialReference || new esri.SpatialReference({ "wkid": 102100 });
        ...
        this.flareColor = options.flareColor || new esri.Color([0, 0, 0, 0.5]);
        ...
        var defaultTextSymbol = new esri.symbols.TextSymbol()
                       .setColor(new esri.Color([255, 255, 255]))
                       .setAlign(esri.symbols.Font.ALIGN_START)
                       .setFont(new esri.symbols.Font("10pt").setWeight(esri.symbols.Font.WEIGHT_BOLD).setFamily("calibri"))
    ...
    onClick: function (evt) {
        ...
        if (evt.graphic.attributes.isCluster) {
            ...
            var p = esri.geometry.webMercatorUtils.geographicToWebMercator(new esri.geometry.Point(flareObject.singleData.x, flareObject.singleData.y, this.spatialRef));
    ...
    add: function (p) {
        ...
        var webExtent = esri.geometry.webMercatorUtils.project(map.extent, new esri.SpatialReference({ "wkid": 102100 }));
        ...
        if (this.spatialRef.isWebMercator()) {
            ...
            web = esri.geometry.webMercatorUtils.lngLatToXY(obj.x, obj.y);
        }
    ...
    _clusterData: function () {
        ...
        //get an extent that is in web mercator to make sure it's flat for extent checking
        var webExtent = esri.geometry.webMercatorUtils.project(this.map.extent, new esri.SpatialReference({ "wkid": 102100 }));
    ...
    _createSingle: function (single) {
        ...
        var point = new esri.geometry.Point(single.x, single.y, this.spatialRef);
        var attributes = dojo._base.lang.clone(single);
        var graphic = new esri.Graphic(point, null, attributes, null);
    ...
    _createCluster: function (cluster) {
        ...
        var point = new esri.geometry.Point(cluster.x, cluster.y, this.spatialRef);
        ...
        if (this.clusterAreaDisplay && cluster.points && cluster.points.length > 0) {
            ...
            var area = esri.geometry.geometryEngine.convexHull(mp, true); //use convex hull on the points to get the boundary
            var areaAttr = dojo._base.lang.clone(attributes);
            areaAttr.isClusterArea = true;
            areaGraphic = new esri.Graphic(area, null, areaAttr, null);
    ...
    _createClusterGraphic: function (cluster) {
        ...
        var create = dojox.gfx.fx.animateTransform({
            duration: 200,
            shape: groupShape,
            transform: [
        ...
        if (this.clusterAreaDisplay === 'always' && areaShape) {
            ...
            var areaCreate = dojox.gfx.fx.animateTransform({
                duration: 200,
                shape: areaShape,
                transform: [
        ...
        //add events
        if (this.flareShowMode === "mouse") {
            this.graphicEvents.push(dojo.on(groupShape, "mouseleave", dojo._base.lang.hitch(this, this._clearActiveCluster)));
        }
    },
    ...
    _activateCluster: function (graphic) {
        ...
        if (this.clusterAreaDisplay === 'hover') {
            cluster.areaGraphic.show();
            var areaDisplay = dojox.gfx.fx.animateTransform({
                duration: 300,
        ...
        var scaleUp = dojox.gfx.fx.animateTransform({
            duration: 400,
            shape: groupShape,
            transform: [
        ...
        var flareSymbol = new esri.symbols.SimpleMarkerSymbol()
                            .setStyle(esri.symbols.SimpleMarkerSymbol.STYLE_CIRCLE)
                            .setSize(radius * 2);
        ...
        for (var i = 0, len = flareCount; i < len; i++) {
            ...
            var sym = dojo._base.lang.clone(flareSymbol);
            sym.setColor(fo.color).setOutline(new esri.symbols.SimpleLineSymbol(esri.symbols.SimpleLineSymbol.STYLE_SOLID, this.textSymbol.color, fo.strokeWidth));
            ...
            var attributes = fo.singleData ? dojo._base.lang.clone(fo.singleData) : {};
            attributes.isFlare = true;
            var flareGraphic = new esri.Graphic(fo.mapPoint, sym, attributes, null);
    ...
    _clearActiveCluster: function (e) {
        ...
        var scaleAnims = [];
        if (this.clusterAreaDisplay === 'hover') {
            var areaHide = dojox.gfx.fx.animateTransform({
                duration: 600,
                shape: cluster.areaGraphic.getShape(),
                transform: [
        ...
        var scaleDown = dojox.gfx.fx.animateTransform({
            duration: 400,
            shape: groupShape,
            transform: [
    ...
    _createTooltip: function (shape) {
        ...
        var rectShape = tooltipGroup.createRect({ x: textBox.x - rectPadding, y: textBox.y - rectPadding, width: textBox.width + (rectPadding * 2), height: textBox.height + (rectPadding * 2), r: 0 })
                        .setFill(new esri.Color([255, 255, 255, 0.9]))
    ...
    _animationEnd: function (layer) {
        ...
        var textElement = dojo.query("> text", this.shape.rawNode).forEach(function (elem) {
            var txt = new dojox.gfx.Text(elem);
