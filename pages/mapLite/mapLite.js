...
window.onload = function () {
    ...
    if (parent.avalon && parent.avalon.vmodels['sszhxt_vm']) {
        tmpPoint = [parent.avalon.vmodels['sszhxt_vm'].$cityDetailobj.lon, parent.avalon.vmodels['sszhxt_vm'].$cityDetailobj.lat];
    }
    map = new MapLite.Map("mapDiv", {
        center: tmpPoint,
        level: 12,
        isInertiaDrag: false,
        basemap: "online/tdt/map"
    });
...
window.esriMap = {
    ...
    addPolyline: function (points, lineSymbol) {
        ...
        let polyLine = new MapLite.LineString(copyarr)
    ...
    createPicSymbol: function (width, height, url) {
        let symbol = new MapLite.PictureSymbol({
            "src": url, //图片访问路径
            "height": height, //高度
            "width": width, //宽度
